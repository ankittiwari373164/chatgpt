...
`;
    ...
    // Append contact details by default (existing clients without the
    // field set behave as if it were enabled).
...
/* Build a clean contact lines block from client phone/email/website.
...
function augmentPrompt(basePrompt, client) {
    ...
    /* ============================================================
       Order matters. Image models give heaviest weight to whatever
       appears earliest. We put HARD RULES first, then the creative
       subject, then attachments + size.
    ============================================================ */

    const parts = [];

    /* ---- 1. HARD RULES (first so the model anchors on these) ---- */
