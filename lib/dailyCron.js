...
/* ============================================================
   ...
   - logo and footer image URLs if the client has them
   - clear instructions to incorporate those assets
============================================================ */
...
function augmentPrompt(basePrompt, client) {
    ...
    parts.push(
        ...
        "- Keep important subjects centered and away from the edges.\n" +
        "- Do NOT add any watermark, signature, or AI-generation badge."
    );

    /* Brand assets (only if URLs are set on the client) */
    const logo   = (client?.logoUrl   || "").trim();
    const footer = (client?.footerUrl || "").trim();

    if (logo || footer) {

        parts.push("\nBRAND ASSETS TO INCORPORATE:");

        if (logo) {
            parts.push(
                "- LOGO IMAGE: " + logo + "\n" +
                "  Use this as the brand's logo. Place it in a top corner of the " +
                "image, sized small and tasteful, not covering important subjects."
            );
        }

        if (footer) {
            parts.push(
                "- FOOTER / BANNER IMAGE: " + footer + "\n" +
                "  Use this as a footer strip across the bottom of the image. " +
                "Keep its style consistent with the rest of the design."
            );
        }

        parts.push(
            "\nThe two URLs above are reference images. Open them, look at them, " +
            "and faithfully include those visual elements in the final composition. " +
            "Do not invent a different logo or footer."
        );
    }
