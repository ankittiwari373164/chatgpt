...
/* ============================================================
   ...
   - reserved space for logo + footer overlay (composited
     server-side AFTER generation by lib/composer.js)
============================================================ */
...
function augmentPrompt(basePrompt, client) {
    ...
    parts.push(
        ...
        "- Do NOT write the brand name as text in the image — branding will be added separately."
    );

    const logo   = (client?.logoUrl   || "").trim();
    const footer = (client?.footerUrl || "").trim();

    /* Reserved-space instructions only if the client has assets to overlay */

    if (logo || footer) {

        parts.push("\nRESERVED SPACE FOR BRANDING (do NOT draw anything here):");

        if (logo) {
            parts.push(
                "- TOP-RIGHT CORNER: leave a clean ~200x200 px area empty " +
                "(no important subjects, no text). A logo will be overlaid there later."
            );
        }

        if (footer) {
            parts.push(
                "- BOTTOM STRIP: keep the bottom ~130 px of the image visually " +
                "simple (no text, no critical subjects). A footer banner will be " +
                "overlaid across the bottom later."
            );
        }

        parts.push(
            "Keep important subjects centered in the canvas and away from those reserved zones."
        );
