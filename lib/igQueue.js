...
async function publishJob(jobId) {
    ...
    try {
        ...
        const isStory = job.mediaType === "story";

        const containerParams = {
            access_token: job.fbToken
        };

        // Stories don't take a caption — IG doesn't support text on
        // API-published Stories. So omit caption for stories.
        if (!isStory) {
        ...
        if (isStory) {
            ...
            // Stories with audio MUST be video. Our storyBuilder
            // always returns an MP4 URL even for image-based stories.
        ...
        if (isStory || isVideo) {
            // Stories are video too — poll like videos
            await pollContainer(containerId, job.fbToken, 180000);
        } else {
